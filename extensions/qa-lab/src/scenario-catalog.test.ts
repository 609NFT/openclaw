...
describe("qa scenario catalog", () => {
  ...
  const agentRuntime = "agent-runtime-and-provider-execution";
  const browserUi = "browser-control-ui-and-webchat";
  const cli = "cli-install-update-onboard-doctor";
  const codex = "openai-codex-provider-path";
  const memory = "session-memory-and-context-engine";
  const otel = "telemetry-diagnostics-and-observability";
  ...
  it("loads the YAML pack as the canonical source of truth", () => {
    ...
    expect(recall.coverage?.primary).toContain(`${memory}.memory-search-and-store-tools-recall`);
  });
  ...
  it("loads helper-backed HTTP API scenarios as supporting taxonomy coverage", () => {
    expect(readQaScenarioById("openai-compatible-chat-tools").coverage?.secondary).toStrictEqual([
      "gateway-runtime.openai-compatible-apis",
    ...
    expect(readQaScenarioById("openai-web-search-minimal").coverage?.secondary).toEqual(
      expect.arrayContaining([
        ...
        "web-search-tools.openai-native-web-search",
        "plugin-sdk-and-bundled-plugin-architecture.web-search-and-fetch",
      ]),
    );
    const webuiCoverage = readQaScenarioById("openwebui-openai-compatible").coverage?.secondary;
    expect(webuiCoverage).toContain("gateway-runtime.openai-compatible-apis");
  ...
  it("loads the QA bus tool trace visibility harness scenario", () => {
    ...
    expect(claims?.primary).toContain(`${otel}.model-and-runtime-telemetry-tool-trace-visibility`);
    expect(claims?.secondary ?? []).toStrictEqual([
      `${otel}.model-and-runtime-telemetry-qa-bus`,
      `${otel}.model-and-runtime-telemetry-trace`,
    ]);
  ...
  it("loads Codex plugin lifecycle scenarios into the standard runtime tier", () => {
    ...
    expect(coldInstall.coverage?.primary).toEqual([
      "plugin-sdk-and-bundled-plugin-architecture.install-update-and-uninstall-hot-install",
    ]);
  ...
  it("routes the Codex doctor migration row through the product-backed Vitest", () => {
    ...
    expect(scenario.coverage?.secondary).toEqual([`${otel}.openclaw-doctor-codex-plugin-auth`]);
  });
