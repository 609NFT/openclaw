...
export const ja_JP: TranslationMap = {
  ...
  channels: {
    ...
    whatsapp: {
      ...
      loggedOut: "ログアウトしました。",
      logoutNotCleared:
        "保存されている WhatsApp セッションはクリアされませんでした。すでに存在しないか、認証ディレクトリの手動クリーンアップが必要な可能性があります。",
    },
  ...
  sessionsView: {
    ...
    changeIcon: "アイコンを変更",
    customEmoji: "カスタム絵文字",
    invalidEmojiIcon: "絵文字を 1 つだけ入力してください。",
    removeIcon: "アイコンを削除",
  ...
  quickSettings: {
    ...
    personal: {
      ...
      profileIdentity: "あなたの Gateway プロファイル",
      ...
      saving: "保存中…",
      uploading: "処理中…",
      profileHint: "JPEG、PNG、または WebP。画像は中央で切り抜かれ、512px にリサイズされます。",
  ...
  subtitles: {
    ...
    apps: "電話、ウォッチ、デスクトップ、ブラウザ向けのコンパニオンアプリ。",
    ...
    advanced: "残りすべての設定セクションと、生ファイルエディタ。",
  ...
  custodian: {
    ...
    subtitleCaretaker: "システムのセットアップと管理。",
    exitSetup: "セットアップを終了",
    newAgent: "新しいエージェント",
    hatchDraft: "起きて、友よ！",
    ...
    history: {
      ...
      opaqueChange: "表示可能な構成パスの変更はなく、書式またはコメントが変更されました。",
      sources: {
    ...
    nudge: {
      ...
      channelAuth: "{channel} の認証が低下しています — 何が起きたか尋ねてください",
      channelDisconnected: "{channel} が切断されました — 何が起きたか尋ねてください",
      channelDegraded: "{channel} の状態が低下しています — 何が起きたか尋ねてください",
  ...
  mcpServers: {
    ...
    nameInvalid:
      "サーバー名には、文字、数字、ドット、ダッシュ、またはアンダースコアを使用できます。",
    targetInvalid:
      "HTTP トランスポートには URL を、stdio には有効なコマンドラインを入力してください。",
    nameTaken: "“{name}” という名前の MCP サーバーはすでに存在します。",
    missing: "MCP server “{name}” が設定内に見つかりませんでした。",
    missingTransport: "トランスポートがありません",
    addedSuccess: "MCP server {name} を追加しました。",
    enabledSuccess: "MCP server {name} を有効にしました。",
    disabledSuccess: "MCP server {name} を無効にしました。",
    removedSuccess: "MCP server {name} を削除しました。",
    configUnavailable: "設定を利用できません。更新してからもう一度お試しください。",
    connectRequired: "MCP server を変更するには、gateway に接続してください。",
    adminRequired: "MCP server の変更には operator.admin アクセスが必要です。",
    enable: "有効化：",
    disable: "無効にする",
  ...
  labsPage: {
    intro:
      "Labs には、リリース間で変更、破損、または廃止される可能性のある実験的な機能が含まれています。",
    sectionTitle: "実験的機能",
    ...
    codeMode: {
      title: "Code Mode",
      description:
        "エージェントが、コンパクトでサンドボックス化された JavaScript ワークフロー内でツールを組み合わせられるようにします。",
    },
    swarm: {
      title: "Swarm",
      description:
        "Code Mode がサブエージェントのグループを並列にオーケストレーションできるようにします。",
  ...
  appsPage: {
    ...
    cards: {
      ios: {
        title: "iPhone",
        desc: "iOSからOpenClawでチャット、通話、アクションの承認、共有ができます。",
      },
      android: {
        title: "Android",
        desc: "Androidスマートフォンを、チャット、カメラ、Canvasを備えたフル機能のOpenClawノードとして利用できます。",
      },
      appleWatch: {
        title: "Apple Watch",
        desc: "手首から一目で確認できるチャットとクイック返信。",
      },
      wearOs: {
        title: "Wear OS",
        desc: "AndroidコンパニオンがOpenClawをあなたのウォッチに拡張します。",
      },
      macos: {
        title: "macOS",
      ...
      chrome: {
        title: "Chrome拡張機能",
        desc: "OpenClawが既存のChromeを操作できるようにします — タブ、ページ、フォーム。",
      },
      plugins: {
  ...
  profilePage: {
    ...
    identity: {
      title: "アイデンティティ",
      description: "このGatewayでのあなたのプロフィールです。",
      ...
      displayName: "表示名",
      displayNameDescription: "このGatewayを使用している他のユーザーに表示されます。",
      linkedEmails: "リンク済みメール",
  ...
  gatewayLogs: {
    ...
    autoFollow: "自動追従",
    file: "ファイル: {file}",
    truncated: "ログ出力は切り詰められました。最新のチャンクを表示しています。",
    empty: "ログエントリはありません。",
  },
  pluginTabs: {
  ...
  board: {
    ...
    emptyTitle: "作業の準備ができた、すっきりしたボード",
    emptyHint:
      "エージェントはここにウィジェットをピン留めできます — ステータスカードを依頼してみてください。",
    ...
    widget: {
      ...
      needsApprovalDetail: "このウィジェットは追加のアクセス権を要求しました。",
      ...
      frameAuthorizationFailed: "更新を繰り返し試行した後、ウィジェットの認証に失敗しました。",
      sandboxOriginRequired:
        "更新を繰り返し試行した後、ウィジェットの認証に失敗しました。Gateway がウィジェットのサンドボックスポートをルーティングしないリバースプロキシまたはトンネルの背後で実行されている場合は、mcp.apps.sandboxOrigin をサンドボックスリスナーにルーティングされた専用の公開オリジンに設定してください。",
      errorTitle: "このウィジェットを読み込めませんでした",
      errorDetail: "問題はこのカード内に限定されています。",
      actionErrorTitle: "ウィジェットの変更に失敗しました",
      actionErrorDetail: "ダッシュボードは前のウィジェットの状態を保持しました。",
      errorShow: "詳細を表示",
      kindMcp: "MCP",
      kindHtml: "HTML",
    },
  ...
  chat: {
    ...
    sessionHeader: {
      ...
      copied: "コピーしました",
      branches: "スレッドの分岐",
      branchSwitchUnavailable: "エージェントが作業中のため、分岐の切り替えは利用できません。",
    ...
    board: {
      ...
      defaultTab: "メイン",
      mockPlaceholder: "ボードビューの継ぎ目 · {tabs} 個のタブ · {widgets} 個のウィジェット",
      mockOverview: "概要",
      mockResearch: "リサーチ",
      mockSessionStatus: "スレッドのステータス",
      mockRecentFindings: "最近の検出結果",
      mockSourceMap: "ソースマップ",
      resetTitle: "会話をリセットしますか？",
      resetDescription:
        "会話のコンテキストがリセットされます。ダッシュボードはそのまま保持されます。",
    },
    catalog: {
    ...
    workspaceConflict: {
      ...
      description:
        "OpenClaw はローカル版を保持し、他のクラウド変更を適用しました。ステージされた結果を確認するか、競合しているパスについてそのバージョンを取り込んでください。",
      morePaths: "他 {count} パス",
      stagedResult: "ステージされたクラウド結果",
      copyStagedResult: "ステージされた結果の参照をコピー",
      inspectCloud: "最初のクラウドバージョンを確認",
      takeCloud: "最初のクラウドバージョンを取り込む",
      copyInspectCommand: "クラウド確認コマンドをコピー",
      copyTakeCommand: "take-cloud コマンドをコピー",
      commandHelp:
        "これらは Bash または zsh（Windows では Git Bash）で実行してください。inspect がパスが存在しないと表示する場合、クラウドで削除されています。確認してローカルパスを手動で削除してください。checkout がファイル/ディレクトリの競合を報告する場合は、妨げになっているローカルパスを移動または削除してから再試行してください。ステージされた参照が見つからない場合、この通知は古いものです。ローカルパスを変更しないでください。",
      commandsUnavailable:
        "このファイル名にはターミナル制御文字が含まれているため、OpenClaw はコピー可能なシェルコマンドを作成しません。ステージされた参照を直接確認し、注意してパスを手動入力してください。",
      dismiss: "ワークスペース競合通知を閉じる",
      eventSender: "クラウドワークスペース",
      eventTitleOne: "クラウド結果が適用され、1件の競合があります",
      eventTitleMany: "クラウド結果が適用され、{count}件の競合があります",
      eventDescription:
        "これらのパスではローカル版が保持されました。その他のクラウドの変更は適用されました。",
    },
    compaction: {
      label: "圧縮された履歴",
    ...
    messages: {
      ...
      rewindUnavailable: "エージェントが作業中のため、巻き戻しは利用できません",
      forkUnavailable: "エージェントが作業中のため、フォークは利用できません",
      showLess: "表示を減らす",
      showMore: "さらに表示",
      unknownDate: "不明な日付",
      voiceNote: "ボイスメモ",
    ...
    modelControls: {
      ...
      reasoning: "推論",
      speed: "速度",
      sessionOverride: "スレッドの上書き設定",
      usingDefault: "Settings のデフォルトを使用中",
    ...
    composer: {
      ...
      cameraInput: "カメラ",
      cameraListUnsupported: "このブラウザではカメラの一覧を表示できません。",
      cameraNoneFound: "カメラが見つかりませんでした。",
      cameraPageInactive: "このページが非アクティブな間はカメラを利用できません。",
      ...
      microphoneAppliesNextSession: "変更は次のTalkセッションを開始したときに適用されます。",
      ...
      holdToRecordSetting: "マイクボタンを長押ししてディクテーション",
      holdToRecordSettingDescription:
        "コンポーザーのマイクボタンを長押しして話し、離すと送信せずにテキストを挿入します。",
      dictationAudioUnsupported:
        "Gatewayがサポートされていないディクテーション音声形式を返しました。",
      dictationBrowserAudioUnsupported:
        "このブラウザーは8 kHzでディクテーション音声をキャプチャできません。",
      dictationConnecting: "ディクテーションを開始しています…",
      dictationDisconnected: "Gatewayが切断されたため、ディクテーションを停止しました。",
      dictationFailed: "ディクテーションに失敗しました。",
      dictationFinalizing: "ディクテーションを終了しています…",
      dictationFinalizationTimedOut:
        "最後の部分的な文字起こしを確定する前にディクテーションが停止しました。",
      dictationProviderUnavailable:
        "ディクテーション用の文字起こしプロバイダーが設定されていません。",
      dictationRecording: "録音中 {elapsed}",
      dictationReleaseToInsert: "離してディクテーションを挿入",
      ...
      selectedCameraUnavailable:
        "選択したカメラは利用できません。別のカメラまたはシステムデフォルトを選択してください。",
